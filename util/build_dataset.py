...
from Util.raw_data_iter import *
import Util
...
class TextClassificationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        label = self.data[i][1]
        txt = self.data[i][2]
        return (idx, self.transforms[0](label), self.transforms[1](txt))
    
    def __getdata__(self, i):
        idx = self.data[i][0]
        label = self.data[i][1]
        txt = self.data[i][2]
        return idx, label, txt
    ...
    ### Returns the index of document that contains the input token
    def get_txtindex_for_token(self,token_id):
        text_idx = []
        for item in self.data:
            txt_vector = self.transforms[1](item[2])
            matched_index = (txt_vector == token_id).nonzero()
            if matched_index.shape[0] > 0:
                text_idx.append(item[0])
        return text_idx
...
def _setup_datasets(dataset_name, root='.data', ngrams=1, vocab=None,
    ...
    raw_data = {'train': [(idx, label, txt) for (idx, label, txt) in train],
                'test': [(idx, label, txt) for (idx, label, txt) in test]}
...
DATASETS = {
    'AG_NEWS': Util.raw_data_iter.AG_NEWS,
    'SogouNews': Util.raw_data_iter.SogouNews,
    'DBpedia': Util.raw_data_iter.DBpedia,
    'YelpReviewPolarity': Util.raw_data_iter.YelpReviewPolarity,
    'YelpReviewFull': Util.raw_data_iter.YelpReviewFull,
    'YahooAnswers': Util.raw_data_iter.YahooAnswers,
    'AmazonReviewPolarity': Util.raw_data_iter.AmazonReviewPolarity,
    'AmazonReviewFull': Util.raw_data_iter.AmazonReviewFull,
    'IMDB': Util.raw_data_iter.IMDB
}
